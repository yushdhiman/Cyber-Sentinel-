...
const { cves, generateLiveAttacks } = require('../data/threatIntel');
...
router.get('/dashboard', (req, res) => {
  ...
  try {
    ...
    // Fallback to vuln scanner result if OS metrics fail
  ...
  res.json({
    stats: {
      ...
      activeDevices: externalDevices.length || Math.floor(Math.random() * 5) + 3,
      ...
      // Extra real fields for the frontend
